...
Ask the release type via the question tool: "What type of release is this?" with options:
...
```bash
platform=$(bash .opencode/skills/release-new/scripts/detect-platform.sh)
```
...
```bash
new_version=$(bash .opencode/skills/release-new/scripts/calc-version.sh <release_type>)
```
...
```bash
notes=$(bash .opencode/skills/release-new/scripts/get-notes.sh "$platform")
```
...
Confirm via the question tool: "Proceed with creating this release?" with options:

- **Yes, publish** — "Create tag and publish release"
