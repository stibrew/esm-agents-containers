...
**Important**: Avoid `$()` command substitution in bash - use sequential steps.
...
After displaying the table, ask the user (use the question tool) whether to show commit details, with these options:

- **All commits** — show full details for each commit
- **None** — skip details
- **Specific commit** — user provides a commit hash to inspect
...
**If "Specific commit"** or user enters a hash directly: Run the same `git show` command for that commit only.
