...
The parent appends to the progress file at these points using `append-progress.sh` (SCRIPTS_DIR) (do not use `cat >>` or direct writes; always append via the script):
...
```
The orchestrator greps these markers at completion and reports them to the user (see the exec SKILL step 13), so the user learns every question the run answered on its own and why.
