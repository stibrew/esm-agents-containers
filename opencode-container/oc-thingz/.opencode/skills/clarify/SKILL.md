...
- System has a genuine issue -> proceed to planning for a fix
...
- Resolution: suggest updating docs, may warrant a small plan
...
When there are multiple valid solutions, present the choices via the question tool:
...
1. **Plan before fixing** - Draft an implementation plan for the chosen approach and get user approval (suggest the plan agent (Tab) or `/plan-make` for a formal plan file)
2. Plan should reflect the scope assessment from Step 1

**CRITICAL**: Do not attempt to fix issues without planning. Always plan first (plan agent or `/plan-make`) for:
- Bug fixes
- Design changes
- Missing features
...
[If real issue]:
This is a real issue that should be addressed. I recommend a planning pass to design a proper fix.

Should I draft a plan for the fix (plan agent or /plan-make)?
```
...
- If it is a real issue -> proceed to planning for a fix
- **Plan first (plan agent or `/plan-make`) when investigation confirms a genuine bug/flaw**
