...
Which approach is most appropriate and why? Do not start implementing: present the approaches, get the user's choice, and agree on a plan first (suggest switching to the plan agent — Tab — for a formal planning pass, or `/plan-make` to write a plan file).
