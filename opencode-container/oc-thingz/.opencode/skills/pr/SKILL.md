...
Ask before posting (question tool):
```
...
Get the PR number from the user's request. If not provided, list recent PRs and ask user to select:
...
Use the **task tool** to invoke the `investigator` subagent for the full analysis (it may run tests and linters but cannot modify the repo). Pass all context the subagent needs in the message:
...
Confirm next step via the question tool:
...
If user selects "Investigate specific finding", launch another targeted `investigator` subagent to dig into the specific area, then ask again.
...
- **Need more context**: launch a targeted `investigator` subagent to investigate, then ask again
...
Load the writing-style skill for proper tone (skill tool):
...
### Ask User via the Question Tool
...
→ Phase 1.5: [question] "Review mode?" → user selects Quick
...
→ Phase 3: present condensed report
→ Phase 4: [question] "Section type change: Accept typed approach?"
  → User: "Accept"
→ User: "draft the review"
